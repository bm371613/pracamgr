...
Ponadto, jeżeli wartość minimaks oznacza pewne zwycięstwo lub nieuchronną przegraną, pogłębianie jest przerywane, dzięki czemu bot szybciej wykonuje ruchy, zwłaszcza pod koniec rozgrywki.
...
\begin{itemize}
\item{\(hash\) --- wartość funkcji haszującej. Jest wykorzystywana do weryfikacji --- wpisu można go użyć tylko, jeżeli \(hash\) jest równe wartości funkcji haszującej rozpatrywanego stanu gry.}
...
Tak więc każdy wariant MCTS miał rozegrać przeciw Minimaksowi jednakową liczbę meczy dwuosobowych w każdej z \(2^2 -2 = 2\) kombinacji, oraz jednakową liczbę meczy czteroosobowych w każdej z \(2^4 - 2 = 14\) kombinacji.
...
Tę i dalsze wizualizacje należy odczytywać następująco: każdej kombinacji parametrów odpowiada pewien prostokąt, którego kolor (zgodnie z legendą) wskazuje jaka część spośród rozegranych meczy została wygrana.
